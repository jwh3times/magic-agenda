...
| `npm run test:e2e`          | CI-oriented smoke, a11y + visual canaries against `E2E_BASE_URL`  |
| `npm run test:e2e:preview`  | Deployed Pages/CSP probes against `E2E_PREVIEW_URL`               |
...
> `npm run bootstrap:private`. Contributors without access to it work from this repo and
> `.env.example` alone — nothing here requires it. See
> [docs/runbooks/maintainer-workstation-recovery.md](./docs/runbooks/maintainer-workstation-recovery.md)
> for the full setup, including 1Password.
