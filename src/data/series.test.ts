...
describe('an Archived Occurrence still occupies its Occurrence Date (#351)', () => {
  /**
   * `series()` with its second Occurrence Completed and Archived. Archive is excluded only at the
   * Board's view seam (#242); everything in this module must keep treating the row as present.
   */
  function withArchivedSecond() {
    const s = series()
    const archived = {
      ...s.tasks[1],
    ...
    return { templates: s.templates, tasks: [s.tasks[0], archived, s.tasks[2]] }
  }

  it('is never regenerated by materialization', () => {
    const s = withArchivedSecond()
    expect(
      pendingInstances(s.templates, s.tasks, '2026-07-01', nextId).map((m) => m.day),
    ).not.toContain('2026-07-08')
    // Positive control: the same board WITHOUT that row does regenerate the date, so the absence
    // above is the Archived row doing its job rather than a window that never reached 07-08.
    const without = s.tasks.filter((task) => task.id !== 'i2')
    expect(
      pendingInstances(s.templates, without, '2026-07-01', nextId).map((m) => m.day),
    ).toContain('2026-07-08')
  })

  it('gains no Excluded Date', () => {
    const s = withArchivedSecond()
    pendingInstances(s.templates, s.tasks, '2026-07-01', nextId)
    expect(s.templates[0].excludedDates).toEqual([])
  })

  it('counts as a surviving Occurrence when the Series is trimmed after it', () => {
    // i1 is gone, so the Archived i2 is the ONLY Occurrence before a cut at i3. Were Archived rows
    // missing from `state.tasks`, the planners would find no survivor and delete the definition,
    // cascading the Archived row away with it.
    const s = withArchivedSecond()
    const state = { templates: s.templates, tasks: [s.tasks[1], s.tasks[2]] }
    const i3 = state.tasks[1]

    const deleted = planDeleteSeriesFrom(state, i3)
    expect(deleted.state.templates).toHaveLength(1)
    expect(deleted.state.templates[0].recurUntil).toBe('2026-07-14')
    expect(deleted.state.tasks.map((task) => task.id)).toContain('i2')
