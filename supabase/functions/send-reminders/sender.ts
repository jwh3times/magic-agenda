import { candidateKey, notificationTitle, type ReminderCandidate } from './domain.ts'
...
export interface ReminderStore {
  ...
  claimTargets(claim: DeliveryClaim, token: string, nowMs: number): Promise<TargetClaim[]>
  targetSucceeded(target: TargetClaim, token: string, nowMs: number): Promise<void>
  targetRetry(
    target: TargetClaim,
    token: string,
    status: number | null,
  ...
  targetDead(target: TargetClaim, token: string, status: number, nowMs: number): Promise<void>
...
export interface PushTransport {
  send(subscription: PushSubscriptionRow, payload: string, topic: string): Promise<void>
}
...
export async function runReminderSender(
  ...
  push: PushTransport,
  nowMs = Date.now(),
): Promise<{ candidates: number; delivered: number; retried: number; dead: number }> {
  ...
  for (const candidate of candidates) {
    ...
    for (const target of targets) {
      try {
        ...
        const failure = error instanceof PushFailure ? error : new PushFailure(String(error), null)
        if (failure.status === 404 || failure.status === 410) {
          ...
          await store.targetRetry(target, token, failure.status, failure.message, nowMs)
          retried++
        }
      }
    }
