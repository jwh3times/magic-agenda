...
class MemoryStore implements ReminderStore {
  ...
  candidates(nowMs: number): Promise<ReminderCandidate[]> {
    const retry = this.targetStatus === "pending" && this.attemptCount > 0 &&
      nowMs >= this.nextAttemptMs;
    return Promise.resolve(
      this.finished || (!retry && nowMs > candidate.dueMomentMs)
        ? []
        : [candidate],
    );
  }
  ...
  finalize(): Promise<void> {
    this.finished = this.targetStatus === "succeeded" ||
      this.targetStatus === "dead";
    this.deliveryClaimed = false;
    return Promise.resolve();
  }
...
Deno.test("concurrent senders produce one successful push", async () => {
  ...
  await Promise.all([
    runReminderSender(store, push, now),
    runReminderSender(store, push, now),
  ]);
...
Deno.test("a transient failure retries only the pending target", async () => {
  ...
  const second = await runReminderSender(
    store,
    push,
    candidate.windowOpensMs + 5 * 60_000,
  );
  const third = await runReminderSender(
    store,
    push,
    candidate.windowOpensMs + 6 * 60_000,
  );
...
Deno.test("a removed subscription cannot hold a retry delivery open", async () => {
  ...
  store.deleted = true;
  await runReminderSender(store, push, candidate.windowOpensMs + 5 * 60_000);
  const after = await runReminderSender(
    store,
    push,
    candidate.windowOpensMs + 6 * 60_000,
  );
