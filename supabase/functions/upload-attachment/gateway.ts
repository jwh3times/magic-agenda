  ...
  return {
    async upload({ userId, boardId, taskId, file, filename, mimeType }) {
      ...
      if (uploadError) {
        ...
        const { error: cancelError } = await admin.rpc('cancel_attachment_upload', {
          p_account_id: userId,
          p_attachment_id: id,
        })
        if (cancelError) {
          console.error('upload-attachment: reservation cleanup failed', cancelError)
        }
...
function reservationError(message: string): Error {
  if (message === 'attachment_byte_quota_exceeded') {
    return new UploadAttachmentRefusal('This Board has reached its 100 MiB attachment limit.')
  }
  if (message === 'attachment_object_quota_exceeded') {
    return new UploadAttachmentRefusal('This Board has reached its 1,000 attachment limit.')
  }
