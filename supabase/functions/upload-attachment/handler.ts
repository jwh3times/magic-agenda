...
export function createHandler({ authenticate, gateway }: Dependencies) {
  return async (req: Request): Promise<Response> => {
    ...
    if (file.size > 10 * 1024 * 1024) {
      return json({ ok: false, error: 'Attachments must be 10 MiB or smaller.' })
    }
    if (Array.from(filename).length === 0 || Array.from(filename).length > 255) {
      return json({ ok: false, error: 'Attachment names must be 1–255 characters.' })
    }
