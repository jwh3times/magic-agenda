...
by hand in six places, which made every such bump red on arrival and stuck — the bot cannot edit
...
setup-cli reads as "latest". Actions still
use full commit SHAs with version comments, updated through Dependabot’s `github-actions` ecosystem;
this changed the `version:` input only, never a `uses:` reference.
