...
- Added Settings controls for an Account reminder lead and independent browser/device enrollment,
  including permission-denied, unsupported-browser, unconfigured-deployment, Automatic Timezone,
  and iOS Home Screen guidance. The service worker now displays bounded Reminder payloads and opens
  Magic Agenda when a notification is selected ([#267](https://github.com/jwh3times/magic-agenda/issues/267)).
...
- Require agent-completed work with remaining human actions to create or update a labeled private
  follow-up issue, verify private board placement, and publish cross-linked step-by-step wiki
  instructions. Apply the shared workflow through docs-updater, end-session, ship, wizard, and
  general agent guidance, including explicit reporting when publication is blocked.
...
- Send Strict-Transport-Security on Pages responses for one year, including subdomains, so browsers
  retain HTTPS after a secure visit. Include the preload directive without submitting the domain
  to browser preload lists (#288).
